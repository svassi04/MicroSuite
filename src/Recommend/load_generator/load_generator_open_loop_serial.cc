...
std::mutex cout_lock, responses_recvd_mutex, start_counter_mutex, outstanding_mutex, global_stats_mutex, kill_ack_lock;
...
class RecommenderServiceClient {
    public:
        ...
        int main(int argc, char** argv) {
            ...
            long query_jump = (queries_size/(qps*time_duration));
            query_id = 0;// query_jump;
            //std::cout << query_jump << " = " << queries_size << " / ( " << qps << " * " << time_duration << " )\n";
            ...
            while (curr_time < exit_time) 
            {
                if (curr_time >= next_time) {
                    //cout_lock.lock();
                    //std::cout << query_id << " q " << query.first << " " << query.second << std::endl;
                    //std::cout << query_id << "\n";
                    //cout_lock.unlock();
...
//                    query_id = rand() % queries_size;
                    query_id = (query_id + query_jump) % queries_size;
