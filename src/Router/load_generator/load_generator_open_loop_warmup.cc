...
class RouterServiceClient {
    public:
        ...
        int main(int argc, char** argv) {
            ...
            std::string prev_key = "-1";
            long skip_counter = 1;
            long times_skip = 0;
            long times_not_skip = 0;
            // This is inelegant. It depents on the
	        for (int q_num=0; q_num < queries.size(); q_num++){
              	std::string key = std::get<0>(queries[q_num]);
             	std::string value = std::get<1>(queries[q_num]);

                while(!key.compare(prev_key) || skip_counter%20 == 0){
//                    std::cout << q_num << " " << key << " " << prev_key << "duplicate\n";
  	            if (!key.compare(prev_key)){
		        std::cout << q_num << " " << key << " " << prev_key << " duplicate\n";
                        q_num++;
                        key = std::get<0>(queries[q_num]);
                        value = std::get<1>(queries[q_num]);
		            }
                    if (key.compare(prev_key) && skip_counter%20 == 0){
//                        std::cout<<q_num << " " << key.compare(prev_key) << " " << skip_counter%20 << " skip\n";
			std::cout << q_num << " " << key << " " << value << " skip\n";
                        skip_counter=1;
                        prev_key = key;
                        times_skip++;
		        ...
		        std::cout << q_num << " " << key << " " << value << "\n";
//		        std::cout << key << " " << prev_key << "\n";
//                std::cout<<q_num << " " << key.compare(prev_key) << " " << skip_counter%20 << "\n";
                prev_key = key;
                skip_counter++;
                times_not_skip++;
                //num_requests->AtomicallyIncrementCount();
	        std::cout << times_not_skip << "\n";
                router_client.Router(key,
                        value,
                        2,
                        false,
                        false);
                //std::this_thread::sleep_for(std::chrono::microseconds(500));  // Adjustable delay
            }
	    std::cout << times_not_skip << " " << times_skip << "\n";
